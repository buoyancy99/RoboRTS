            ...
            if (executor_state != BehaviorState::RUNNING) {
                ...
                if (std::sqrt(std::pow(dx, 2) + std::pow(dy, 2)) >= 1.0 &&
                    ...
                    reduce_goal.pose.position.x =
                            chase_buffer_[(chase_count_ + 2 - 1) % 2].pose.position.x - 1.2 * cos(yaw);
                    reduce_goal.pose.position.y =
                            chase_buffer_[(chase_count_ + 2 - 1) % 2].pose.position.y - 1.2 * sin(yaw);
                    ...
                    if (blackboard_->GetCostMap2D()->GetCost(goal_cell_x, goal_cell_y) >= 253) {
                        ...
                        for (FastLineIterator line(goal_cell_x, goal_cell_y, robot_cell_x,
                                                   robot_cell_x); line.IsValid(); line.Advance()) {

                            auto point_cost = blackboard_->GetCostMap2D()->GetCost((unsigned int) (line.GetX()),
                                                                                   (unsigned int) (line.GetY())); //current point's cost
