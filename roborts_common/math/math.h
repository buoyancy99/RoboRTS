...
//use cross judge convex
    ...
    inline double PointToLineDistance(const Eigen::Ref<const Eigen::Vector2d> &point,
        ...
        double u = ((point.x() - line_start.x()) * diff.coeffRef(0) + (point.y() - line_start.y()) * diff.coeffRef(1)) /
                   sq_norm;
    ...
    inline double
    DistanceSegmentToSegment2D(const geometry::LineSegment2D &line0, const geometry::LineSegment2D &line1) {
    ...
    inline double
    DistancePointToPolygon2D(const Eigen::Vector2d &point, const std::vector <Eigen::Vector2d> &vertices) {
        ...
        if (vertices.size() > 2) {
            double new_dist = PointToLineDistance(point.coeffRef(0), point.coeffRef(1), vertices.back().coeffRef(0),
                                                  vertices.back().coeffRef(1),
