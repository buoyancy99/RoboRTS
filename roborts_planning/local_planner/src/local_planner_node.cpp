...
namespace roborts_local_planner {
    ...
    roborts_common::ErrorInfo LocalPlannerNode::Init() {
        ...
        std::string full_path =
                ros::package::getPath("roborts_planning") + "/local_planner/config/local_planner.prototxt";
    ...
    void LocalPlannerNode::Loop() {
        ...
        while (GetNodeState() == NodeState::RUNNING) {
            ...
            if (error_info.IsOK()) {
                ...
                error_info = roborts_common::ErrorInfo(roborts_common::ErrorCode::LP_MAX_ERROR_FAILURE,
                                                       "over max error.");
                SetNodeState(NodeState::FAILURE);
            } else {
                error_count++;
