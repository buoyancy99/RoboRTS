            ...
            if (container_ptr) {
                ...
                if (buffer_pool_map_.count(std::make_pair(container_ptr->command_info.cmd_set,
                    ...
                    DLOG_INFO << "Capture command: "
                              ...
                              << int(container_ptr->command_info.cmd_set)
                              << ", cmd id: 0x" << std::setw(2) << std::hex << std::setfill('0')
                              << int(container_ptr->command_info.cmd_id)
                              << ", sender: 0x" << std::setw(2) << std::hex << std::setfill('0')
                              << int(container_ptr->command_info.sender)
                              << ", receiver: 0x" << std::setw(2) << std::hex << std::setfill('0')
                              << int(container_ptr->command_info.receiver);
...
//      DLOG_EVERY_N(ERROR, 100)<<"nothing to take";
            ...
            if (container.message_header.is_ack) {
                if (int(container.command_info.receiver) != int(command_info->sender)) {
                    DLOG_ERROR << "Requested ACK receiver: " << std::setw(2) << std::hex << std::setfill('0')
                               << int(command_info->sender)
                               << ", Get ACK receiver: " << std::setw(2) << std::hex << std::setfill('0')
                               << int(container.command_info.receiver);
                    mismatch = true;
                }
                if (int(container.command_info.sender) != int(command_info->receiver)) {
                    DLOG_ERROR << "Requested ACK sender: " << std::setw(2) << std::hex << std::setfill('0')
                               << int(command_info->receiver)
                               << ", Get ACK sender: " << std::setw(2) << std::hex << std::setfill('0')
                ...
                if (int(container.command_info.sender) != int(command_info->sender)) {
                    DLOG_ERROR << "Requested sender: " << std::setw(2) << std::hex << std::setfill('0')
                               << int(command_info->sender)
    ...
    bool Protocol::SendCMD(uint8_t cmd_set, uint8_t cmd_id, uint8_t receiver,
        ...
        switch (session_mode) {
            ...
            case CMDSessionMode::CMD_SESSION_0:
                ...
                memcpy(cmd_session_ptr->memory_block_ptr->memory_ptr + pack_length - CRC_DATA_LEN, &crc_data,
                       CRC_DATA_LEN);
    ...
    bool Protocol::SendACK(uint8_t session_id, uint16_t seq_num, uint8_t receiver,
        ...
        if (session_id == 0 || session_id > 31) {
            ...
            if (ack_ptr != nullptr && ack_length != 0) {
                ...
                memcpy(ack_session_ptr->memory_block_ptr->memory_ptr + pack_length - CRC_DATA_LEN, &crc_data,
                       CRC_DATA_LEN);
            }
                        ...
                        switch (ack_session_table_[header_ptr->receiver][header_ptr->session_id - 1].session_status) {
                            case ACKSessionStatus::ACK_SESSION_IDLE:
                                ...
                                if (header_ptr->session_id > 1) {
                                    memory_pool_ptr_->LockMemory();
                                    ack_session_table_[header_ptr->receiver][header_ptr->session_id -
                                                                             1].session_status =
                            ...
                            case ACKSessionStatus::ACK_SESSION_USING:
                                ...
                                memory_pool_ptr_->LockMemory();
                                session_header_ptr = (Header *) ack_session_table_[header_ptr->receiver][
                                        header_ptr->session_id
                                ...
                                if (session_header_ptr->seq_num == header_ptr->seq_num) {
                                    ...
                                    recv_container_ptr_->command_info.length =
                                            header_ptr->length - HEADER_LEN - CMD_SET_PREFIX_LEN - CRC_DATA_LEN;
                                    recv_container_ptr_->command_info.cmd_set = *((uint8_t *) header_ptr + HEADER_LEN +
                                                                                  1);
                                    recv_container_ptr_->command_info.cmd_id = *((uint8_t *) header_ptr + HEADER_LEN);
                                    recv_container_ptr_->command_info.need_ack = true;
                                    memcpy(recv_container_ptr_->message_data.raw_data,
